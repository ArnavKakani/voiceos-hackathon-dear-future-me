...
const states = {
    idle: {
        layers: [
            { 
                color: 0x5D8E67,  // Light purple
                opacity: 0.2,
                scale: 1.0,
                rotationSpeed: { x: 0.001, y: 0.002, z: 0 }
            },
            { 
                color: 0x5D8E67,  // Medium purple
                opacity: 0.2,
                scale: 0.85,
    ...
    listening: {
        layers: [
            { 
                color: 0x9FD89C,  // Soft blue
                opacity: 0.2,
                scale: 1.0,
                rotationSpeed: { x: 0.002, y: 0.004, z: 0 }
            },
            { 
                color: 0x9FD89C,  // Medium blue
                opacity: 0.2,
                scale: 0.85,
                rotationSpeed: { x: -0.003, y: 0.005, z: 0.002 }
            },
            { 
                color: 0x9FD89C,  // Light blue
                opacity: 0.4,
                scale: 0.70,
    ...
    thinking: {
        layers: [
            { 
                color: 0xFEE188,  // Soft purple
                opacity: 0.2,
                scale: 0.85,
                rotationSpeed: { x: 0.003, y: 0.003, z: 0 }
            },
            { 
                color: 0xFEE188,  // Medium purple
                opacity: 0.2,
                scale: 0.72,
                rotationSpeed: { x: -0.004, y: 0.004, z: 0.002 }
            },
            { 
                color: 0xFEE188,  // Light purple
                opacity: 0.4,
                scale: 0.60,
    ...
    speaking: {
        layers: [
            { 
                color: 0x3A5C42,  // Soft pink
                opacity: 0.2,
                scale: 1.0,
                rotationSpeed: { x: 0.004, y: 0.005, z: 0 }
            },
            { 
                color: 0x3A5C42,  // Medium pink-red
                opacity: 0.2,
                scale: 0.85,
                rotationSpeed: { x: -0.005, y: 0.006, z: 0.003 }
            },
            { 
                color: 0x3A5C42,  // Bright pink
                opacity: 0.4,
                scale: 0.70,
...
const fragmentShader = `
    ...
    void main() {
        ...
        color = mix(color, rainbow, holographicIntensity * 0.18);
...
// The canvas is FULL-SCREEN (see VoiceSessionView.swift), so the orb's size on
// screen is set purely by how far away the camera stands. With a 75° vertical
// fov the visible world height at distance d is 2*d*tan(37.5°) ≈ 1.5347*d, and
// the outermost sphere is 2.0 world units across. Solve that for the diameter
// we actually want in CSS px:
//
//     d = 2 * canvasHeight / (1.5347 * targetDiameterPx)
//
// The target mirrors the tap-target circle in VoiceSessionView.swift
// (`min(width * 0.57, 240)`) so the visible orb and the thing you can tap are
// the same size on every device. Keep the two formulas in sync.
const ORB_FOV_HEIGHT_FACTOR = 1.53466;   // 2 * tan(75° / 2)
const ORB_SPHERE_DIAMETER = 2.0;         // outermost layer, world units

function orbCameraDistance(w, h) {
    const target = Math.min(w * 0.57, 240.0);
    if (!target || !h) return 4.96;
    return (ORB_SPHERE_DIAMETER * h) / (ORB_FOV_HEIGHT_FACTOR * target);
}
...
function init() {
    ...
    // Camera
    const _c0 = document.getElementById('canvas');
    const _w0 = _c0.clientWidth || window.innerWidth, _h0 = _c0.clientHeight || window.innerHeight;
    camera = new THREE.PerspectiveCamera(75, _w0 / _h0, 0.1, 1000);
    camera.position.z = orbCameraDistance(_w0, _h0);
    ...
    const pointLight1 = new THREE.PointLight(0x9FD89C, 0.6, 100);
    pointLight1.position.set(5, 5, 5);
    scene.add(pointLight1);
    
    const pointLight2 = new THREE.PointLight(0x5D8E67, 0.4, 100);
...
// Handle resize — read the canvas's ACTUAL size (not window.inner*/vh, which
// don't match a small WKWebView frame), and re-fire when SwiftUI resizes us.
function onWindowResize() {
    const c = document.getElementById('canvas');
    const w = c.clientWidth || window.innerWidth, h = c.clientHeight || window.innerHeight;
    if (!w || !h) return;
    camera.aspect = w / h;
    camera.position.z = orbCameraDistance(w, h);
    camera.updateProjectionMatrix();
    renderer.setSize(w, h, false);
}
try {
    const _ro = new ResizeObserver(() => onWindowResize());
    _ro.observe(document.getElementById('canvas'));
} catch (e) {}
...
// Generate speech-like cadence for the visual speaking state. This affects
// animation only. Audio is owned exclusively by the native ElevenLabs player.
...
function animate() {
    ...
    if (state.pulsate) {
        if (state.pulsateMode === 'audio-reactive') {
            ...
            // SPEAKING: visual cadence only; this web view never emits audio.
            const cadenceIntensity = updateSpeakingCadence();
